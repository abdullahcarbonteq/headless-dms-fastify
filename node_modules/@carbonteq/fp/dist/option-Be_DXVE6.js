...
var Option = class Option {
	...
	isNone() {
		return this.val === NONE_VAL || isPromise(this.val) && this.#ctx.promiseNoneSlot;
	}
	isUnit() {
	...
	flatMap(mapper) {
			...
			const newP = new Promise((resolve, reject) => {
				p.then((curr$1) => {
					if (curr$1 === NONE_VAL) {
						...
						if (isPromise(r)) r.then((innerOpt) => resolve(innerOpt.val), reject);
						else resolve(r.val);
					}
				});
			});
		...
		if (isPromise(mapped)) return Option.fromPromise(mapped);
		return mapped;
	}
	zip(fn) {
		if (this.isNone()) return Option.None;
...
//#endregion
export { Option, UnwrappedNone };
//# sourceMappingURL=option-Be_DXVE6.js.map