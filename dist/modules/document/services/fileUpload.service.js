    ...
    async processFileUpload(fileData) {
        try {
            ...
            if (fileData.file.size > config.app.upload.maxFileSize) {
                return Result.Err(new Error(`File too large. Maximum size is ${config.app.upload.maxFileSize / (1024 * 1024)}MB`));
            }
            // Validate file type
            if (!config.app.upload.allowedMimeTypes.includes(fileData.file.mimetype)) {
                return Result.Err(new Error(`File type not allowed. Allowed types: ${config.app.upload.allowedMimeTypes.join(', ')}`));
            }
            // Parse tags
            let tagsString = '[]';
            if (fileData.fields.tags) {
                try {
                    ...
                    tagsString = JSON.stringify(fileData.fields.tags.split(',').map((t) => t.trim()));
                }
            }
